---
layout: article
title: "Android 系统架构 —— 系统启动篇 之 应用进程的启动"
key: "Android 系统架构 —— 系统启动篇 之 应用进程的启动" 
...
## 前言
在上一篇文章中, 我们知道系统服务进程中所有的服务启动完毕后, 会通知 AMS 启动 Launcher 应用, 既然是一个应用程序, 那么自然是运行在一个单独的应用进程中

这里我们先忽略 Activity 启动的流程, 看看如何启动一个应用进程
...
### 回顾
![向 Zygote 发起创建请求](https://i.loli.net/2019/10/19/HtoJybIKEMaj1Lc.png)
