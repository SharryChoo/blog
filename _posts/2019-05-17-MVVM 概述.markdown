...
- ViewModel(视图数据): 用于给 View 层处理和提供数据
...
- ViewModel 是为了解决 MVP 中 Presenter 过重这个问题而生的, ViewModel 和 View 之间的通信并不需要通过接口来回进行数据的导入导出, 而是在 ViewModel 中暴露数据让 View 的观察, 当数据变更时会自动的反应到与之绑定的 View 上
...
### UML 图
![MVVM 架构设计](https://note.youdao.com/yws/res/87084/62581B9EE49F485DB5CAAC5E029408FA)
