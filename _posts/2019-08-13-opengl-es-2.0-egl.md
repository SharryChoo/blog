...
## 什么是 EGL?
OpenGL ES 是操作 GPU 的 API, 它无法直接与屏幕设备进行交互
...
至此当前线程的 EGL 的环境便搭建好了, 我们知道 OpenGL 绘制之后是需要输出的, 这个输出的缓冲区即为 EGLSurface, 结下来看看缓冲区的相关操作
...
现在 EGLSurface 与 EGLContext 已经绑定好了, 现在就可以安心的执行 OpengGL 的渲染管线了, 也就是我们业务代码的编写
...
- 从上一次重绘区域上拷贝数据
- 使用 nativeWindow(Surface/SurfaceFlinger) 将这个缓冲区到 SurfaceFlinger 待绘制队列
...
- 执行 OpenGL ES 的渲染管线 
- /////////////////////////////////////////
- 交换缓冲, 将 EGLSurface 中的数据推送到 SurfaceFlinger 渲染队列
  - 推送之后, 会立即获取一个新的缓冲, 即双缓冲机制 
- 绘制结束后销毁当前线程的 EGL 数据
